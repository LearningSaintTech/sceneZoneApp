...
const ExploreEventScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    const fetchEventDetails = async () => {
      ...
      try {
        console.log(`Making GET request to http://192.168.1.52:3000/api/host/get-event/${eventId}`);
        const response = await axios.get(`http://192.168.1.52:3000/api/host/events/get-event/${eventId}`, {
          headers: {
