...
const SignUpScreen = ({ navigation }) => {
  ...
  const validateInputs = () => {
    ...
    if (!sanitizedLocation || sanitizedLocation.length < 2) {
      setCustomAlert({ visible: true, title: 'Error', message: 'Please enter a valid location (at least 2 characters)' });
      return false;
    }
    ...
    if (!password.trim() || password.length < 6) {
      setCustomAlert({ visible: true, title: 'Error', message: 'Please enter a password (at least 6 characters)' });
      return false;
    }
  ...
  const handleSignup = async () => {
    ...
    const signupData = {
     ...
     try {
      ...
      const response = await api.post('https://api.thescenezone.com/api/host/firebase-auth/firebase-signup', signupData);
