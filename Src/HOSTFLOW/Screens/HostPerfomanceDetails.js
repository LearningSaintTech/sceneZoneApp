...
//       try {
//         const response = await axios.get(`http://192.168.1.9:3000/api/artist/get-artist-performance/${artist.artistId}`, {
//           headers: {
...
const HostPerfomanceDetailsScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const fetchPerformances = async () => {
      ...
      try {
        const response = await axios.get(`http://10.0.2.2:3000/api/artist/get-artist-performance/${artist.artistId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
