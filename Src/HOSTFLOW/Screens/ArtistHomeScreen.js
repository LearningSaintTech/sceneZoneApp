...
const ArtistEventCard = ({ item, navigation, onEventApplied, onEventSaved, savedEvents }) => {
  const [showGuestListInput, setShowGuestListInput] = useState(false);
  const [isApplying, setIsApplying] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [localAppliedStatus, setLocalAppliedStatus] = useState(item.isApplied || false);
  const [localSavedStatus, setLocalSavedStatus] = useState(savedEvents.has(item.id) || false);
  const dispatch = useDispatch();
  const token = useSelector(state => state.auth.token);
  
  // Update local status when item prop changes (to sync with parent updates)
  useEffect(() => {
    setLocalAppliedStatus(item.isApplied || false);
  }, [item.isApplied]);
  ...
  // Check if this event is already applied (from API response or local state)
  ...
  const handleApplyPress = async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        // Update local applied status immediately and permanently
      ...
      const errorMessage = error.response?.data?.message || 'Failed to apply for event.';
  ...
  const handleSavePress = async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        Alert.alert('Failed', response.data?.message || 'Failed to save event.');
      }
    } catch (error) {
          ...
          >
            {isAlreadyApplied ? (
              <View style={styles.appliedButtonContainer}>
                <Icon name="check" size={16} color="#B15CDE" />
...
const ArtistHomeScreen = ({ navigation }) => {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const response = await api.get('/artist/events/get-all-events-artist', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        }
      ...
      if (response.data && response.data.data) {
        ...
        const transformedEvents = response.data.data.map((event, index) => {
          ...
          // Check if applied from backend OR locally applied
          const isApplied = event.isAppliedByCurrentArtist || 
                           event.isApplied || 
                           locallyAppliedEvents.has(event._id) || 
                           false;
  ...
  const loadAppliedEvents = async () => {
    try {
      ...
      if (appliedEventsData) {
        ...
        console.log('Loaded applied events from storage:', appliedEventsArray);
      }
    } catch (error) {
  ...
  const handleEventApplied = (eventId) => {
    ...
    console.log('Event applied successfully and saved to storage:', eventId);
  };
  ...
  const handleEventApplicationDeleted = (eventId) => {
    ...
    console.log('Event application deleted and removed from storage:', eventId);
  };
