...
const EventDashboardScreen = ({ navigation, route }) => {
  ...
  const toggleGuestList = async () => {
    ...
    try {
      console.log(`Making POST request to http://192.168.1.52:3000/api/host/events/toggle-guest-list/${eventId}`);
      const response = await axios.patch(
        `http://192.168.1.52:3000/api/host/events/toggle-guest-list/${eventId}`,
