...
const HostEditProfileScreen = ({ navigation }) => {
  ...
  const fetchProfileData = async () => {
    ...
    try {
      ...
      if (error.response) {
        console.error(
          "ProfileScreen: Error response data:",
          error.response.data
        );
        console.error(
      ...
      console.log(
        "ProfileScreen: fetchProfile completed, setting loading to false"
      );
      setLoading(false);
    }
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      console.log(
        "Sending request to /host/auth/updateHost with token:",
        token
      );
      console.log(
        "Form data being sent (cannot view content directly):",
        formData
      );

      const response = await api.put("/host/auth/updateHost", formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        ...
        <View style={styles.inputContainer}>
          ...
          <View style={styles.emailInputWrapper}>
            <TextInput
              style={[
                styles.input,
                { flex: 1, borderColor: "transparent", borderWidth: 0 },
              ]}
              value={email}
