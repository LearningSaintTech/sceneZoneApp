...
const EventScreen = ({ navigation }) => {
  ...
  const fetchEvents = useCallback(async () => {
    try {
      const response = await axios.get('http://192.168.1.9:3000/api/host/events/get-all-events', {
        headers: {
  ...
  const handleDeleteEvent = async (eventId) => {
    try {
      const response = await axios.delete(`http://192.168.1.9:3000/api/host/events/delete-event/${eventId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
