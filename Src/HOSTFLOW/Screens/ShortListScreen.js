...
const ShortlistScreen = ({ navigation }) => {
  ...
  const [isContractDetailsModalVisible, setContractDetailsModalVisible] = useState(false);
  const [isAddToExistingEventsModalVisible, setAddToExistingEventsModalVisible] = useState(false);
      ...
      const fetchShortlistedArtists = async () => {
        ...
        try {
          console.log('Fetching shortlisted artists from API', {
            url: 'http://192.168.1.52:3000/api/host/getShortlistedArtists',
            headers: { Authorization: `Bearer ${token}` },
          });
          const response = await axios.get('http://192.168.1.52:3000/api/host/getShortlistedArtists', {
            headers: {
  ...
  const formatEventDate = (dateArr) => {
    ...
    const formatted = `${date.toLocaleString('en-US', { month: 'short' })} ${date.getDate()}`;
      ...
      const fetchManageEvents = async () => {
        ...
        try {
          console.log('Fetching events from API', {
            url: 'http://192.168.1.52:3000/api/host/events/get-all-events',
            headers: { Authorization: `Bearer ${token}` },
          });
  ...
  const toggleShortlist = async (id) => {
    ...
    try {
      console.log('Sending DELETE request', {
        url: `http://192.168.1.52:3000/api/host/removeShortlistArtist/${item.artistProfile.artistId}`,
        headers: { Authorization: `Bearer ${token}` },
      });
      const response = await axios.delete(`http://192.168.1.52:3000/api/host/removeShortlistArtist/${item.artistProfile.artistId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      console.log('Updated apiData', apiData.filter(i => i.artistProfile._id !== id));
      Alert.alert('Success', `Artist with ID ${item.artistProfile.artistId} removed from shortlist`);
    } catch (error) {
      console.error('Error response:', error.response?.data || error.message);
      Alert.alert('Error', `Failed to remove artist with ID ${item.artistProfile.artistId}`);
    }
    console.log('Removal process completed for artist ID:', item.artistProfile.artistId);
  };
  
  const renderShortlistItem = ({ item }) => {
    console.log('renderShortlistItem called', { item });
  
    console.log(`Attempting to render image for item ${item.artistProfile._id} with URI: ${item.artistProfile.profileImageUrl}`);
  
    return (
      <TouchableOpacity
        style={styles.eventCard}
        onPress={() => {
          console.log('Navigating to HostPerfomanceDetails', { artistId: item.artistProfile.artistId });
          navigation.navigate('HostPerfomanceDetails', { artistId: item.artistProfile.artistId });
        }}
        activeOpacity={0.8}
      >
        <View style={styles.imageContainer}>
          <Image
            source={{ uri: item.artistProfile.profileImageUrl }}
            style={styles.eventImage}
            resizeMode="cover"
            onLoad={() => console.log(`Image successfully loaded for item ${item.artistProfile._id}`)}
            onError={(e) => console.error(`Failed to load image for item ${item.artistProfile._id}. URI: ${item.artistProfile.profileImageUrl}. Error:`, e.nativeEvent.error)}
          />
          <View style={styles.overlayRow}>
            <TouchableOpacity
              style={styles.overlayRemove}
              onPress={() => {
                console.log('Remove from shortlist pressed', { id: item.artistProfile._id });
            ...
            <View style={[styles.overlayButton, styles.overlayButtonFirst]}>
              <Text style={styles.overlayButtonText}>{item.artistProfile.genre ? item.artistProfile.genre[0]?.toUpperCase() : 'ARTIST'}</Text>
            </View>
            <View style={styles.overlayButton}>
              <Text style={styles.overlayButtonText}>{`$${item.artistProfile.budget}`}</Text>
            </View>
            <TouchableOpacity
              style={styles.overlayPlus}
              onPress={() => {
                console.log('Add to shortlist pressed, opening add options modal');
                setSelectedArtist(item.artistProfile); // Store selected artist
  ...
  const renderManageEventCard = (event) => {
    console.log('renderManageEventCard called', { event });
    return (
      <View style={styles.manageEventCardContainer} key={event._id}>
        <View style={styles.manageEventImageWrapper}>
          <Image
            ...
            resizeMode="cover"
            onLoad={() => console.log(`Event image loaded for event ${event._id}`)}
            onError={(e) => console.error(`Failed to load event image for event ${event._id}. URI: ${event.posterUrl}`, e.nativeEvent.error)}
          />
        ...
        style={styles.manageEventButtonPurple}
          >
            <TouchableOpacity
              onPress={() => {
                console.log('Navigating to HostManageEvent', { eventId: event._id });
                navigation.navigate('HostManageEvent',{ eventId: event._id });
              }}
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              console.log('Delete event pressed', { eventId: event._id });
              if (!event._id) {
              ...
              try {
                console.log('Sending DELETE request for event', {
                  url: `http://192.168.1.52:3000/api/host/events/delete-event/${event._id}`,
                  headers: { Authorization: `Bearer ${token}` },
                });
                setManageEventsLoading(true);
                const response = await axios.delete(
                  `http://192.168.1.52:3000/api/host/events/delete-event/${event._id}`,
                  {
                    headers: {
                      Authorization: `Bearer ${token}`,
                      'Content-Type': 'application/json',
                ...
                console.log('Delete event API response', response.data);
                setManageEvents(prev => prev.filter(e => e._id !== event._id));
                console.log('Updated manageEvents', manageEvents.filter(e => e._id !== event._id));
                Alert.alert('Success', 'Event deleted successfully');
              } catch (error) {
                console.error('Error deleting event:', error.response?.data || error.message);
                Alert.alert('Error', 'Failed to delete event');
              } finally {
                console.log('Delete event completed, manageEventsLoading:', false);
                setManageEventsLoading(false);
              }
            }}
          >
  ...
  const fetchExistingEvents = async () => {
    ...
    try {
      ...
      console.error('Error fetching existing events:', error.response?.data || error.message);
      setExistingEvents([]);
    } finally {
      console.log('fetchExistingEvents completed, existingEventsLoading:', false);
      setExistingEventsLoading(false);
    }
  };

  console.log('Rendering ShortlistScreen', { activeTab, apiData, manageEvents });
          ...
          <View style={styles.listContainer}>
            {loading ? (
              ...
              <FlatList
                ...
                keyExtractor={item => item.artistProfile._id}
                ListEmptyComponent={<Text style={{ color: '#fff', textAlign: 'center', marginTop: 40 }}>No artists found.</Text>}
              />
            )}
          </View>
        ) : (
            ...
            {manageEventsLoading ? (
              ...
              manageEvents.length > 0 ? (
                manageEvents.map(event => {
                  console.log('Rendering manage event card', { event });
                  return renderManageEventCard(event);
                })
              ) : (
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                console.log('On Salary Basis pressed, opening contract details modal');
                setAddOptionsModalVisible(false);
            ...
            <TouchableOpacity
              style={styles.modalButton}
              onPress={() => {
                console.log('Add To Existing Events pressed, opening existing events modal');
                setAddOptionsModalVisible(false);
            ...
            <TouchableOpacity
              style={styles.modalButton}
              onPress={() => {
                console.log('Navigating to ShortlistCreateNewEvent with artist', selectedArtist);
                navigation.navigate('ShortlistCreateNewEvent', { artist: selectedArtist });
                setAddOptionsModalVisible(false);
              }}
            >
        ...
        <View style={styles.modalOverlay}>
          <View style={[
            ...
            <View style={[
              ...
              <TouchableOpacity
                onPress={() => {
                  console.log('Back button pressed, closing contract details modal');
                  setContractDetailsModalVisible(false);
                }}
          ...
          existingEvents.map((item) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => {
                  console.log('Navigating to HostDetailBdjbvjdhvjdhfvjrfhvjfhvjookingScreen', {
              ...
              >
                <Image
                  ...
                  onLoad={() => console.log(`Existing event image loaded for event ${item._id}`)}
                  onError={(e) => console.error(`Failed to load existing event image for event ${item._id}. URI: ${item.posterUrl}`, e.nativeEvent.error)}
                />
