...
const ShortlistScreen = ({ navigation }) => {
  ...
  const toggleShortlist = async (id) => {
    ...
    if (shortlistedItems[id]) {
      // Show confirmation pop-up before removing
      Alert.alert(
        'Confirm Removal',
        `Are you sure you want to remove the artist with ID ${item.artistId}?`,
        [
          {
            text: 'Cancel',
            style: 'cancel',
          },
          {
            text: 'Remove',
            onPress: async () => {
              console.log("Starting removal process for artist ID:", item.artistId);
              try {
                console.log("Sending DELETE request to:", `http://localhost:3000/api/host/removeShortlistArtist/${item.artistId}`);
                console.log("Headers:", { 'Authorization': `Bearer ${token}` });
                const response = await axios.delete(`http://localhost:3000/api/host/removeShortlistArtist/${item.artistId}`, {
                  headers: {
                    'Authorization': `Bearer ${token}`,
                  },
                });
                ...
                setApiData(prev => prev.filter(i => i._id !== id));
                Alert.alert('Success', `Artist with ID ${item.artistId} removed from shortlist`);
              } catch (error) {
                console.error("Error response:", error.response?.data || error.message);
                Alert.alert('Error', `Failed to remove artist with ID ${item.artistId}`);
              }
              console.log("Removal process completed for artist ID:", item.artistId);
            },
            style: 'destructive',
          },
        ]
